...
import { OauthImpl, BufferUtits, TokensHelper, BaseTokenPayload, validatePayload } from "../../helpers";
import { oauth } from "./oauth-config";
import { Authentication, Authorization } from "../../helpers/middleware/oauth";
import { AuthValidator } from "../../helpers/validations";
import { SuccessResponse, ErrorResponse, StatusCodes, ErrorCodes } from "../../../response";

const OauthController = Router();

const handleError = (err: any, req: Request, res: Response, response: OauthResponse): void => {
    ...
    res.send(new ErrorResponse(err.message, innerError && innerError.errorCode ? innerError.errorCode : null));
};
...
export const logoutHandler = async (req: Request, res: Response): Promise<void> => {
    ...
    if (!token) {
        res.status(StatusCodes.UNAUTHORIZED_ACCESS).send(new ErrorResponse("Access Token Not Found", ErrorCodes.access_token_missing));
    } else {
        const user = TokensHelper.decodeToken(token) as BaseTokenPayload;
...
OauthController.post('/authenticate', AuthValidator.login(), validatePayload, Authentication, authHandler);
